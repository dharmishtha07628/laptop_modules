...
class CrossoveredBudget(models.Model):
    ...
    def export_budget_lines(self):
        # Define CSV export logic
        data = io.StringIO()
        writer = csv.writer(data)
        # Write header
        writer.writerow(
            ['BUDGET REPORTING GROUP', 'ANALYTIC ACCOUNT', 'START DATE', 'END DATE', 'PAID DATE', 'PLANNED AMOUNT',
             'PRACTICAL AMOUNT', 'THEORETICAL AMOUNT', 'ACHIEVEMENT'])  # Replace with actual field names
        # Write data
        for line in self.crossovered_budget_line:  # Adjust according to your model
            writer.writerow(
                [line.general_budget_id.name, line.analytic_account_id.name, line.date_from, line.date_to, line.paid_date,
                 line.planned_amount, line.practical_amount, line.theoritical_amount,
                 line.percentage])  # Replace with actual field names
        data.seek(0)
        file_data = base64.b64encode(data.getvalue().encode()).decode()
        attachment = self.env['ir.attachment'].create({
            'name': 'Budget_Lines.csv',
            'type': 'binary',
            'datas': file_data,
            'mimetype': 'text/csv',
        })
    ...
    # def action_import_budget_lines(self):
    #     context = self.env.context.copy()
    #     context['import_budget_lines'] = True
    #     return {
...
class CrossoveredBudgetLines(models.Model):
    ...
    general_budget_id = fields.Many2one('account.budget.post', 'Budget Reporting Group')
    date_from = fields.Date('Start Date', required=True)
    date_to = fields.Date('End Date', required=True)
    ...
    def get_import_templates(self):
        return [{
            'label': _('Import Template for Budget Lines'),
            'template': '/om_account_budget/static/sample_file/Sample_Budget_Line.xlsx'
        }]
