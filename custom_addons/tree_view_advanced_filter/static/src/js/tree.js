...
export const ListRendererCust = {
    ...
    _renderSearchPanel: async function () {
        ...
        if (tableRef.hasAttribute('style')) {
            tableRef.removeAttribute('style');
        }
        let dataNames = Array.from(tableRef.querySelectorAll('[data-name]')).map(element => element.getAttribute('data-name'));
...
//                return false;
//            }
            // MUNAF: remove following conditions for correct list order
            ...
            if (!dataNames.includes(col.name)) {
                return false
            }
            return true;
        });
        ...
        if (this.hasSelectors) {
            $tr1.prepend($('<td>').addClass('o_list_record_selector bg-light'));
        }

        if (this.isStudioEditable || this.displayOptionalFields) {
            $tr1.append($('<td>').addClass('o_list_record_selector bg-light'));
        }
        ...
        rootRef.querySelectorAll('.input_select').forEach((a) => a.addEventListener('change', this._onSearch.bind(this)));
    },
    ...
    _renderHeaderCellSearch: function (node) {
        ...
        var $input = $('<input>').attr({
            ...
            'class': 'input_text min-wp-50 bg-light'
        });
        var search_button = $('<button>').attr({
            type: 'button',
            name: name,
            'class': 'search_button invisible'
        });
            ...
            if (string !== " " && field.store) {
                if (field.type == 'selection') {
                    ...
                    if (value_of_tr) {
                        ...
                        value_of_tr.forEach((val) => {
                            ...
                            $value_tag.css('border-radius', '10px');
                            // $value_tag.css('font-size', '12px');
                ...
                if (field.type == 'datetime' || field.type == 'date') {
                    ...
                    if ($(search_button).hasClass('hidden_button')) {
                        ...
                        $input_date1.css('border-radius', '10px');
                    }
