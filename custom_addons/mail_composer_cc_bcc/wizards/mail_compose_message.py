...
class AccountMoveSend(models.TransientModel):
    ...
    partner_cc_ids = fields.Many2many(
        "res.partner",
        "partner_cc_res_partner_rel",
        "account_move_cc_send_id",  # This model's column
        "partner_id",  # Related model's column (res.partner)
        string="Cc",
        readonly=False,
        store=True,
    )
    ...
    partner_bcc_ids = fields.Many2many(
        "res.partner",
        "partner_bcc_res_partner_rel",
        "account_move_bcc_send_id",  # This model's column
        "partner_id",  # Related model's column (res.partner)
        string="Bcc",
        readonly=False,
        store=True,
    ...
    @api.model
    def default_get(self, fields_list):
        """ Set default CC and BCC partners from the company settings. """
        company = self.env.company
        res = super().default_get(fields_list)
        partner_cc = company.default_partner_cc_ids
        if partner_cc:
    ...
    def _get_mail_params(self, move, move_data):
        """ Override to add CC & BCC recipients in mail params. """
        print("=-=-c=cc,bcc")

        # Call the original method from the parent class
        mail_params = super()._get_mail_params(move, move_data)

        # Get CC & BCC partner IDs correctly from move_data
        partner_cc_ids = self.partner_cc_ids.ids
        partner_bcc_ids = self.partner_bcc_ids.ids

        if partner_cc_ids and hasattr(partner_cc_ids, "ids"):
            partner_cc_ids = partner_cc_ids.ids  # Convert recordset to list of IDs

        if partner_bcc_ids and hasattr(partner_bcc_ids, "ids"):
            partner_bcc_ids = partner_bcc_ids.ids  # Convert recordset to list of IDs

        # Add CC & BCC recipients to mail params
        mail_params.update({
            "recipient_cc_ids": partner_cc_ids,
            "recipient_bcc_ids": partner_bcc_ids,
        })
        print("-=-=cc bcc email body-=-=-")

        return mail_params

    def _get_mail_move_values(self, move, wizard=None):
        """ Override to add CC & BCC recipients in mail move values. """

        # Call the original method from the parent class
        mail_values = super()._get_mail_move_values(move, wizard)

        # Get CC & BCC partner IDs correctly
        ...
        # Add CC & BCC recipients to mail values
        if mail_values:
            mail_values.update({
                "recipient_cc_ids": partner_cc_ids,
                "recipient_bcc_ids": partner_bcc_ids,
