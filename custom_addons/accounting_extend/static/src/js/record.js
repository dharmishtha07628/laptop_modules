    ...
    _checkValidity({ silent, displayNotification } = {}) {
        ...
        if (!isValid && displayNotification) {
            const items = [...this._invalidFields].map((fieldName) => {
                if(this.resModel=='account.move' && fieldName=='partner_id'){
                    if (['out_invoice', 'out_refund', 'out_receipt'].includes(this.data['move_type'])){
                        this.fields[fieldName].string = 'Customer';
                    }
                    if (['in_invoice', 'in_refund', 'in_receipt'].includes(this.data['move_type'])){
                        this.fields[fieldName].string = 'Vendor';
                    }
                }
